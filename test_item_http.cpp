...
TEST(HttpTest, GetTotalFile) {
	...
	std::optional<std::vector<monitor_client::common_utility::FileInfo>> result = item_http.GetTotalFile();
...
TEST(HttpTest, GetFile) {
	...
	std::optional<monitor_client::common_utility::FileInfo> result = item_http.GetFile(current_file_info.name);
...
TEST(HttpTest, RemovedFile) {
	...
	bool result = item_http.RemoveFile(file_name);
	ASSERT_TRUE(result);
}
...
TEST(HttpTest, RenamedFile) {
	...
	bool result = item_http.RenameFile(info);
	ASSERT_TRUE(result);
}